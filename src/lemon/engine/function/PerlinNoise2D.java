...
public class PerlinNoise2D implements BinaryOperator<Float> {
	...
	public PerlinNoise2D(IntUnaryOperator hasher, IntBinaryOperator pairer, BinaryOperator<Float> persistence,
			int iterations) {
		this.hasher = hasher;
		this.pairer = pairer;
	...
	public float smoothHash2D(int x, int y) {
		return (hash2D(x - 1, y - 1) + hash2D(x - 1, y + 1) + hash2D(x + 1, y - 1) + hash2D(x + 1, y + 1)) / 16
				+ (hash2D(x - 1, y) + hash2D(x + 1, y) + hash2D(x, y - 1) + hash2D(x, y + 1)) / 8 + hash2D(x, y) / 4;
	}
